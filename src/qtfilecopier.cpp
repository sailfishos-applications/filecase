...
struct CopyFileNode : public ChainNode {
    ...
    bool handle() {
        ...
        double freeSize = 0;
        double TargetSize = 0;
        QString nPath = r.dest;
        int i = nPath.lastIndexOf( "/" );
        nPath.remove ( i, nPath.length() - i );
        if ( nPath == "" ) nPath = "/";
        QByteArray ba = nPath.toLatin1();
        const char *str1 = ba.data();
        statfs ( str1, &stat );
        freeSize = stat.f_bsize*stat.f_bfree;
        TargetSize = QFileInfo ( r.source ).size();
        if ( freeSize < TargetSize )
        {
            setError(QtFileCopier::NoEnoughSpace); // no enough space on target
            return false;
        }
        ...
        if (error() != QtFileCopier::NoError)
            ...
            copiado.append(sourceFile.fileName());
            settings.setValue("CopiedFiles", copiado );
            settings.sync();*/
        }
        return done;
    }
