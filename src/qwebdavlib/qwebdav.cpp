                                    ...
                                    const QString &sslCertDigestSha1)
{
    ...
    /*QString newhost = hostname;

    if (newhost.contains("/")) {
        int i = newhost.indexOf("/");
        m_prePath = newhost;
        m_prePath = m_prePath.remove(0, i);
        newhost = newhost.left(i);
    }*/
    ...
    qDebug() << hostname << m_baseUrl.host();
...
QNetworkReply* QWebdav::createRequest(const QString& method, QNetworkRequest& req, QIODevice* outgoingData, bool addSize)
{
    if(outgoingData!=0 && outgoingData->size()!=0 && addSize) {
...
QNetworkReply* QWebdav::createRequest(const QString& method, QNetworkRequest& req, const QByteArray& outgoingData, bool addSize)
{
    ...
    QNetworkReply* reply = createRequest(method, req, dataIO, addSize);
...
{
#ifdef DEBUG_WEBDAV
    qDebug() << "QWebdav::list() path = " << m_baseUrl.host() << m_prePath << path;
#endif
    return list(path, 1);
}
...
QNetworkReply* QWebdav::get(const QString& path, QIODevice* data, quint64 fromRangeInBytes)
{
    ...
    connect(reply, SIGNAL(finished()), this, SLOT(replyReadyRead()));
    connect(reply, SIGNAL(error(QNetworkReply::NetworkError)), this, SLOT(replyError(QNetworkReply::NetworkError)));
    connect(reply, SIGNAL(downloadProgress(qint64, qint64)), this, SIGNAL(fileProgress(qint64,qint64)));
...
QNetworkReply* QWebdav::propfind2(QString path, QStringList props)
{
    QByteArray query;
    ...
    return createRequest("PROPFIND", req, query, false);
}
